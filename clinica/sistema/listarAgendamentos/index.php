...
try {
  $sql = <<<SQL
    select ta.data_, ta.horario, ta.nome as paciente, ta.sexo, ta.email, tm.especialidade, tp.nome
      from ((tb_agenda ta inner join tb_medico tm on medico_id = tm.id) 
	    inner join tb_pessoa tp on tp.id = tm.id);
SQL;
  $stmt = $pdo->query($sql);
} catch (Exception $e) {
...
<head>
  ...
  <title>Todos agendamentos</title>
</head>
<body class="bg-black">
<header>
